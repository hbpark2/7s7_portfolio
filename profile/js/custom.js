...
window.addEventListener('scroll', function () {
    ...
    //기존의 li의 class를 지우고 현재 섹션의 li에만 class를 부여한다.
    ...
    for (let b = 0; b < section.length; b++) {
        section[b].addEventListener(browser, function (e) {
            ...
            if (browser == 'mousewheel') {
                ...
                lastTop = section[section.length - 1].offsetTop;

                if (wheelData > 0 && currTop > 960) {
                    prev = this.previousElementSibling.offsetTop;
                    window.scrollTo({
                        top: prev,
                        behavior: 'smooth'
                    });
                } else if (wheelData < 0 && currTop < lastTop) {
                    next = this.nextElementSibling.offsetTop;
                    window.scrollTo({
                        top: next,
                        behavior: 'smooth'
                    });
...
// 이 스크립트는 다양한 플러그인을 써보고 익힐 용도로 사용했습니다.
